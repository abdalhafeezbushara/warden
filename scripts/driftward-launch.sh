#!/bin/bash
#
# driftward-launch.sh — do the two launch steps for Driftward:
...
#   (B) verify Driftward's --deep recording live on this Mac (needs Full Disk
#       Access, which you grant once in System Settings — see below).
#
# HOW TO RUN:
#   1. Open Terminal.
#   2. chmod +x ~/Desktop/driftward/scripts/driftward-launch.sh
#   3. ~/Desktop/driftward/scripts/driftward-launch.sh
#
...
#   N=80  ~/Desktop/driftward/scripts/driftward-launch.sh     # fetch 80 servers (default 40)
...
# config
# ----------------------------------------------------------------------------
DRIFTWARD_REPO="${DRIFTWARD_REPO:-$HOME/Desktop/driftward}"
N="${N:-40}"
STAMP="$(date +%Y%m%d-%H%M%S)"
WORK="$HOME/Desktop/driftward-scan-$STAMP"
CORPUS="$WORK/corpus"
export DRIFTWARD_HOME="$WORK/.driftward"
...
# ----------------------------------------------------------------------------
# locate python (need 3.11+) and the driftward repo
# ----------------------------------------------------------------------------
PY=""
...
if [ ! -d "$DRIFTWARD_REPO/driftward" ]; then
  warn "Can't find the Driftward code at $DRIFTWARD_REPO."
  warn "Set DRIFTWARD_REPO=/path/to/driftward and re-run."
  exit 1
fi
DRIFTWARD() { ( cd "$DRIFTWARD_REPO" && "$PY" -m driftward "$@" ); }
ok "Using $PY ($($PY --version 2>&1)) and Driftward at $DRIFTWARD_REPO"
...
PYEOF
  ...
  if [ "$import_count" -gt 0 ]; then
    ...
    DRIFTWARD scan "$CORPUS" --static-only \
...
say "PART B — verifying Driftward --deep recording on this Mac"
cat <<'NOTE'
...
CANARY="/tmp/driftward-deep-canary-$STAMP"
DRIFTWARD run --deep -- sh -c "cat \"$HOME/.gitconfig\" >/dev/null 2>&1; /usr/bin/touch \"$CANARY\"" || true

say "What the deep recorder captured:"
LATEST="$(ls -t "$DRIFTWARD_HOME"/sessions/*.log 2>/dev/null | head -1)"
if [ -n "$LATEST" ]; then
  "$PY" - "$LATEST" <<'PYEOF'
import json, sys
counts = {}
