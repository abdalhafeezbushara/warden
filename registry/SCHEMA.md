# Registry entry format (`driftward.registry-entry/v1`)
...
```json
{
  "schema": "driftward.registry-entry/v1",
  "subject": {
  ...
  "provenance": {
    "reviewer": "driftward-maintainers",
    "reviewed_at": 1234567890.0,
    "source": "https://github.com/.../driftward-registry",
    "notes": "GitHub MCP server: reaches only api.github.com.",
    "driftward_version": "0.2.0"
  },
  "policy": { "...optional reviewed Driftward policy (see `driftward run --policy`)..." },
  "signature": {
    "algorithm": "ed25519",
    "public_key": "<64-hex>",
    "value": "<128-hex>",
...
- **subject** — what the entry is about. `kind` is `mcp`, `skill`, `agent`, or
  `command`; `name` is how you install it (`driftward registry install <name>`).
...
- **capabilities** — the reviewed, normalized behavior in five categories. This
  ...
  optional `port` (443 for HTTPS), matching how Driftward records real egress.
...
- **provenance** — reviewer, review source/notes, and the Driftward version used.
- **policy** — optional. A reviewed least-privilege policy document, installable
  with `--policy-out`.
- **signature** — Ed25519 over the canonical payload.

Adopting an entry (`driftward registry install`) verifies the signature, checks the
