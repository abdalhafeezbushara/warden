#!/bin/bash
#
# driftward-detonate.sh — DYNAMIC detonation of MCP servers in disposable Docker
# containers. Each server is installed and run under Driftward inside a throwaway
...
# HTTP_PROXY) and Driftward's proxy (verdicts for proxy-honoring clients).
#
...
# PREREQUISITES:
#   - Docker running.
#   - Image built once:  cd ~/Desktop/driftward && docker build -f detonate/Dockerfile -t driftward-detonate .
#
...
IMAGE="${IMAGE:-driftward-detonate}"
RUNSECS="${RUNSECS:-15}"
STAMP="$(date +%Y%m%d-%H%M%S)"
OUT="$HOME/Desktop/driftward-detonate-$STAMP"
...
docker image inspect "$IMAGE" >/dev/null 2>&1 || {
  ...
  ( cd "$DRIFTWARD_REPO" && docker build -f detonate/Dockerfile -t "$IMAGE" . ) || { warn "build failed"; exit 1; }
}
...
MCP_INIT='{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2024-11-05","capabilities":{},"clientInfo":{"name":"driftward","version":"1"}}}
...
if [ -n "$BIN" ]; then
  printf "%s" "$MCP_INIT" | runuser -u detonator -- env DRIFTWARD_HOME=/tmp/driftward PYTHONPATH=/opt/driftward \
      timeout "${RUNSECS}s" python3 -m driftward record -- "$BIN" >/tmp/rec.log 2>&1 || true
fi
sleep 1
python3 - <<PYEOF
import json,sys,re,glob,os
sys.path.insert(0,"/opt/driftward")
...
proxy=set()
os.environ["DRIFTWARD_HOME"]="/tmp/driftward"
try:
...
python3 - "$RESULTS" "$OUT" "$DRIFTWARD_REPO" <<'PYEOF'
import json, sys, os
