...
from driftward import childenv, proxy, seatbelt
from driftward.policy import Policy, NetworkRules, FilesystemRules
...
class EnvironmentScrubbing(unittest.TestCase):
    ...
    def test_driftward_control_vars_never_leak(self):
        os.environ["DRIFTWARD_HOME"] = "/x"
        os.environ["DRIFTWARD_MCP_TOKEN"] = "broker-secret"
        env = childenv.build_child_env(
            Policy(name="t", env_allow=["DRIFTWARD_HOME", "DRIFTWARD_MCP_TOKEN"]), {})
        self.assertNotIn("DRIFTWARD_HOME", env)
        self.assertNotIn("DRIFTWARD_MCP_TOKEN", env)
...
class ControlPlaneDeny(unittest.TestCase):
    def test_driftward_home_denied_when_injected(self):
        # runner.run injects the driftward home into deny; simulate that and confirm
        # the compiler emits a denial covering it.
        home = "/Users/x/.driftward"
        pol = Policy(name="t", filesystem=FilesystemRules(deny=[home + "/**"]))
        prof = seatbelt.compile_profile(pol, 0)
        self.assertIn("/.driftward", prof)


class ProfilePolicy(unittest.TestCase):
    def test_profiling_policy_confines_writes(self):
        from driftward.profiler import _profiling_policy
...
class FailClosedEnforcement(unittest.TestCase):
    def test_run_does_not_start_child_when_backend_is_unavailable(self):
        from driftward import backends, report, runner, sessions

        root = Path(tempfile.mkdtemp(prefix="driftward-fail-closed-"))
        marker = root / "child-ran"
        old_home = os.environ.get("DRIFTWARD_HOME")
        os.environ["DRIFTWARD_HOME"] = str(root / "home")
        try:
            ...
            if old_home is None:
                ...
                os.environ["DRIFTWARD_HOME"] = old_home
            import shutil
...
class KeychainAccess(unittest.TestCase):
    ...
    def test_cursor_baseline_opens_keychain(self):
        ...
        from driftward.policy import KEYCHAIN_GLOB, keychain_allowed
    ...
    def test_default_policy_denies_keychain(self):
        from driftward.policy import default_policy, keychain_allowed
        self.assertFalse(keychain_allowed(default_policy("/tmp/proj")))

    def test_apply_keychain_toggles_and_is_idempotent(self):
        from driftward.policy import (Policy, FilesystemRules, KEYCHAIN_GLOB,
    ...
    def test_keychain_readable_under_strict_read(self):
        ...
        from driftward import agents, seatbelt
