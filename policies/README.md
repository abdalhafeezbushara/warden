...
Reviewed, least-privilege policies you can point Warden at.

- `agents/` — reference policies matching the built-in agent baselines. Useful
  as a starting point to copy into a project `.warden.yaml` and tighten.
...
```bash
warden run --policy policies/skills/example-fetch-skill.yaml -- <command>
```
...
```bash
warden profile ./the-skill.sh --out policies/skills/the-skill.yaml
```
