...
it, and it relays to the server's Streamable-HTTP endpoint. Run under Driftward with
...
class _Bridge:
    def __init__(self, url: str, headers: dict | None = None, timeout: float = 300.0):
        ...
        self._init_request: str | None = None   # remembered for session recovery
    ...
    def _post(self, body: bytes):
        ...
        # After initialization the client MUST echo the negotiated protocol
        # version on every request (MCP Streamable HTTP transport).
        if self.protocol_version:
            request.add_header(PROTOCOL_HEADER, self.protocol_version)
        return urllib.request.urlopen(request, timeout=self.timeout)

    def _capture_protocol(self, message: str) -> None:
        try:
            version = json.loads(message).get("result", {}).get("protocolVersion")
    ...
    def _reinitialize(self) -> bool:
        """Replay the initialize handshake to recover an expired session (a 404 on
        a request with a session id means the server dropped it). Silent — the
        agent already saw the first initialize response."""
        if not self._init_request:
            return False
        self.session_id = None
        try:
            response = self._post(self._init_request.encode("utf-8"))
        ...
        with response:
            ...
            messages = (list(_sse_messages(response)) if "text/event-stream" in ctype
                        else [response.read().decode("utf-8", "replace")])
            for message in messages:
                self._capture_protocol(message)
    ...
    def _error_reply(self, request_id, message: str) -> None:
        ...
        self._write(json.dumps({
            ...
            "error": {"code": -32001, "message": f"driftward bridge: {message}"}}))
    ...
    def _handle(self, line: str) -> None:
        try:
            ...
            try:
                ...
                request_id, method = parsed.get("id"), parsed.get("method")
            except (ValueError, AttributeError):
                pass
            try:
                response = self._post(line.encode("utf-8"))
            except urllib.error.HTTPError as exc:
                # 404 on a session request = the server expired our session;
                # re-initialize once and retry, per the transport spec.
                if exc.code == 404 and method != "initialize" and self._reinitialize():
                    try:
                        response = self._post(line.encode("utf-8"))
                    except (urllib.error.HTTPError, urllib.error.URLError, OSError) as retry_exc:
                        self._error_reply(request_id, f"endpoint error after re-init ({retry_exc})")
                        return
                else:
    ...
    def run(self) -> int:
        ...
        for raw in sys.stdin.buffer:
            ...
            if not self._initialized:
                if method == "initialize":
                    self._init_request = line   # remembered for session recovery
                self._handle(line)
                if method == "initialize":
...
def _bridge_sse(url: str, headers: dict, timeout: float) -> int:
    ...
    threading.Thread(target=reader, name="driftward-mcp-sse", daemon=True).start()
    ...
    for raw in sys.stdin.buffer:
        ...
        try:
            ...
            print(f"driftward mcp bridge: POST failed: {exc}", file=sys.stderr)
