...
Driftward deliberately separates three ideas that security tools often blur:
...
only so other open-source tools can produce and verify it without Driftward Cloud.
"""
...
MANIFEST_SCHEMA = "driftward.behavior/v1"
BASELINE_SCHEMA = "driftward.baseline/v1"
...
def baselines_dir() -> Path:
    path = _driftward_home() / "baselines"
...
def diff(manifest: dict, baseline: dict) -> dict:
    ...
    return {
        "schema": "driftward.behavior-diff/v1",
