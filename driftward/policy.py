"""Driftward policy model and loader.
...
and what to do on a violation. To keep Driftward dependency-free, policies are
...
def apply_keychain(policy: Policy, allow: bool) -> Policy:
    ...
    Driftward denies the keychain by default so an agent can't read the user's other
...
def default_policy(workdir: str | os.PathLike[str] | None = None) -> Policy:
    ...
    return Policy(
        name="default",
        description="Driftward default: project-scoped filesystem, allow-listed egress, secrets denied.",
