...
Driftward looks for a project policy so a team can commit one `.driftward.yaml` and
have every `driftward run` in that tree use it. Discovery walks up from the working
...
PROJECT_FILENAMES = (".driftward.yaml", ".driftward.yml", ".driftward/policy.yaml")
