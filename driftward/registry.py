...
No network, no Driftward Cloud: a registry is just a directory of signed JSON files
...
ENTRY_SCHEMA = "driftward.registry-entry/v1"
...
def _registry_home() -> Path:
    base = Path(os.environ.get("DRIFTWARD_HOME", Path.home() / ".driftward")) / "registry"
...
def entry_trust(entry: dict) -> tuple[bool, str, str]:
    ...
    if not is_trusted(signer):
        return False, (f"signer {signer[:12]}… is not trusted — add it with "
                       "`driftward registry trust <key>` if you have reviewed it"), signer
