...
record breaks the chain from that point on, so `driftward verify` can prove whether a
...
def verify_log(path: str | Path, expect_pubkey: str | None = None) -> tuple[bool, str]:
    ...
    how a third party verifies a receipt against a known Driftward identity.
    """
