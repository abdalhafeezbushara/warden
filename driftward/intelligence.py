...
Driftward records what an agent did; this module reasons about whether it looks
right. Three capabilities:
  ...
  * observations   — a legacy per-agent fingerprint of previously seen hosts.
    ...
    baselines live in :mod:`driftward.behavior`.
...
# lives in driftward.behavior; merely seeing a capability must never make it trusted.

def _baseline_dir() -> Path:
    base = Path(os.environ.get("DRIFTWARD_HOME", Path.home() / ".driftward")) / "observations"
