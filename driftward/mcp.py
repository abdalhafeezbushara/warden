...
Wrapped servers do not start a nested Driftward inside the agent sandbox. They
connect to a parent-owned broker which was given an immutable allow-list before
the agent started. The broker launches each stdio server as a sibling Driftward
session, preserving Driftward's private signing state while giving every server an
...
class McpServer:
    ...
    def to_grant(self) -> dict:
        """Private broker payload. Stored only below DRIFTWARD_HOME with mode 0600."""
        return {
...
def _wrapper_details(command, args) -> dict | None:
    """Recognize current broker shims and legacy nested-Driftward wrappers."""
    args = [str(a) for a in (args or [])]
    if command != "driftward":
        return None
...
def remote_endpoints(extra_configs=None) -> list[dict]:
    ...
    Driftward cannot sandbox someone else's endpoint, but it can allow-list and
...
def policy_for(server: McpServer, workdir: str) -> Policy:
    ...
    return Policy(
        ...
        description=f"Strict Driftward baseline for MCP server '{server.name}'.",
...
def _driftward_package_root() -> str:
    import driftward
    return str(Path(driftward.__file__).resolve().parent.parent)
...
def remote_policy_for(server: McpServer, workdir: str) -> Policy:
    ...
    so a rug-pulled URL is blocked, not just recorded. Reads include Driftward's own
    ...
    return Policy(
        ...
        filesystem=FilesystemRules(
            read=[workdir + "/**", _driftward_package_root() + "/**",
                  "~/.cache/**", "~/.local/**"],
...
def bridge_command(server: McpServer) -> list[str]:
    """The sandboxed process that bridges stdio to the remote endpoint. The URL is
    NOT on argv — it can carry query tokens or signed credentials, and argv is
    recorded in the session log. It travels through a private env var instead."""
    import sys
    return [sys.executable, "-m", "driftward", "mcp", "_bridge", "--transport", server.transport]
