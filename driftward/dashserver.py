"""Local, read-only dashboard for Driftward sessions.

A dependency-free HTTP server bound to 127.0.0.1 on a fresh port. It serves
static assets from driftward/dashboard/ and a small JSON API backed by the recorded
session logs. It reads only; it never mutates a session or reaches the network.

Mirrors the safety posture of the rest of Driftward: loopback only, no remote
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            # only the hostnames Driftward itself prints/uses for this exact port.
...
def serve(open_browser: bool = True) -> None:
    ...
    url = server.url
    print(f"Driftward dashboard: {url}")
