"""`driftward doctor` — verify Driftward actually works on this machine.

Most security tools ask you to trust that they enforce. Driftward proves it: the
...
def run() -> int:
    ...
    print("Driftward doctor — checking this machine\n")
    ...
    if backend in ("seatbelt", "bubblewrap"):
        if _live_enforcement_test():
            ...
            _p(mark("bad"), "Live enforcement test FAILED — a denied path was still readable",
               "Driftward could not enforce on this machine. Do not rely on it here.")
            bad += 1

    # 4. Signing round-trip
    try:
        seed, pk = crypto.ensure_key()
        sig = crypto.sign(b"driftward-doctor", seed, pk)
        if crypto.verify(sig, b"driftward-doctor", pk) and not crypto.verify(sig, b"x", pk):
    ...
    if bad:
        print("Driftward is NOT fully functional on this machine. See failures above.")
        return 1
    print("Driftward is ready.")
    return 0
...
def _live_enforcement_test() -> bool:
    ...
    tmp = Path(tempfile.mkdtemp(prefix="driftward-doctor-", dir=root))
    try:
        secret_dir = tmp / "secret"
        secret_dir.mkdir()
        secret = secret_dir / "k.txt"
        token = "DRIFTWARD-DOCTOR-CANARY-4f2a"
