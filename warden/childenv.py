...
them. Warden instead passes only a safe base set, credentials registered for the
...
WARDEN_CONTROL_VARS = {
    "WARDEN_HOME", "WARDEN_SESSION", "WARDEN_ACTIVE",
    "WARDEN_MCP_BROKER", "WARDEN_MCP_TOKEN",
}
...
def build_child_env(policy, overrides: dict[str, str], agent: str | None = None) -> dict[str, str]:
    ...
    # Never leak Warden's own control-plane vars to the child.
    for k in WARDEN_CONTROL_VARS:
        env.pop(k, None)
...
def scrubbed_names(policy, agent: str | None = None) -> list[str]:
    ...
    allow = (set(SAFE_BASE) | provider_keys_for(agent)
             | set(getattr(policy, "env_allow", []) or []))
    return sorted(k for k in os.environ if k not in allow and k not in WARDEN_CONTROL_VARS)
