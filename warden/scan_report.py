...
def render_html(agg: dict, title: str = "AI Agent Skill Behavior Scan") -> str:
    ...
    stats = "".join([
        ...
        _stat(agg["detonated"], "safely detonated"),
    ])
...
<h1>{t}</h1>
<p class="sub">Each skill was analyzed statically and, where runnable, <b>safely
detonated under Warden</b> — filesystem protected, network egress contained and
recorded so nothing left the machine. Percentages are of {agg['total']} skills.</p>
...
under containment; the rest were analyzed statically only.</div>
</div></body></html>"""
