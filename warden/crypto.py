...
Warden's hash chain makes a log tamper-*evident* (you can tell it was altered).
...
because Warden signs its own local logs with a local key — there is no remote
...
def _key_dir() -> Path:
    base = Path(os.environ.get("WARDEN_HOME", Path.home() / ".warden"))
