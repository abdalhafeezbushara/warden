...
BROKER_ENV = "WARDEN_MCP_BROKER"
TOKEN_ENV = "WARDEN_MCP_TOKEN"
...
class McpBroker:
    ...
    def __init__(self, servers: list[McpServer], *, recorder=None,
        ...
        self._thread = threading.Thread(target=self._serve, name="warden-mcp-broker", daemon=True)
    ...
    def _serve(self) -> None:
        while not self._stop.is_set():
            ...
            thread = threading.Thread(target=self._handle, args=(conn,),
                                      name="warden-mcp-client", daemon=True)
    ...
    def _grant_file(self, server: McpServer) -> Path:
        ...
        directory = _warden_home() / "mcp-grants"
    ...
    def _inner_command(self, server: McpServer, grant: Path) -> list[str]:
        ...
        return [sys.executable, "-m", "warden", "mcp", "_serve", "--grant", str(grant)]
...
def run_shim(name: str, source: str, definition: str) -> int:
    ...
    if not address or not token:
        raise RuntimeError(
            "no parent MCP broker is available; run the agent through `warden run` "
            "or launch this server with `warden mcp run`")
    host, separator, raw_port = address.rpartition(":")
    if not separator or host not in ("127.0.0.1", "localhost"):
        raise RuntimeError("invalid Warden MCP broker address")
    ...
    thread = threading.Thread(target=upload, name="warden-mcp-stdin", daemon=True)
    thread.start()
    try:
        while True:
